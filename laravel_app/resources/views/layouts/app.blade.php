...
<nav class="sidebar">
  ...
  <div class="sidebar-footer">
    <div class="user-info">
      <div class="avatar">{{ strtoupper(substr(session('prenom','?'),0,1).substr(session('nom','?'),0,1)) }}</div>
      <div>
        <div class="user-nom">{{ session('prenom') }} {{ session('nom') }}</div>
        <div class="user-role">@yield('role-label')</div>
      </div>
    </div>
